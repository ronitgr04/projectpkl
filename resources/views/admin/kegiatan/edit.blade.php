    ...
    <div class="bg-white m-6 rounded-lg shadow">
        ...
        <div class="p-6 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-semibold text-gray-900">Edit Kegiatan</h2>
                    <p class="text-sm text-gray-600 mt-1">Edit kegiatan {{ $kegiatan->mahasiswa->nama ?? 'mahasiswa' }}</p>
                </div>
        ...
        <!-- Form -->
        <div class="p-6">
            <form action="{{ route('admin.kegiatan.update', $kegiatan->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="mahasiswa_id" id="mahasiswa_id" required
                            ...
                            @foreach($mahasiswa as $mhs)
                                <option value="{{ $mhs->id }}" {{ (old('mahasiswa_id', $kegiatan->mahasiswa_id) == $mhs->id) ? 'selected' : '' }}>
                                    {{ $mhs->nama }} - {{ $mhs->nim }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="hari" id="hari" required
                            ...
                            <option value="Senin" {{ (old('hari', $kegiatan->hari) == 'Senin') ? 'selected' : '' }}>Senin</option>
                            <option value="Selasa" {{ (old('hari', $kegiatan->hari) == 'Selasa') ? 'selected' : '' }}>Selasa</option>
                            <option value="Rabu" {{ (old('hari', $kegiatan->hari) == 'Rabu') ? 'selected' : '' }}>Rabu</option>
                            <option value="Kamis" {{ (old('hari', $kegiatan->hari) == 'Kamis') ? 'selected' : '' }}>Kamis</option>
                            <option value="Jumat" {{ (old('hari', $kegiatan->hari) == 'Jumat') ? 'selected' : '' }}>Jumat</option>
                            <option value="Sabtu" {{ (old('hari', $kegiatan->hari) == 'Sabtu') ? 'selected' : '' }}>Sabtu</option>
                            <option value="Minggu" {{ (old('hari', $kegiatan->hari) == 'Minggu') ? 'selected' : '' }}>Minggu</option>
                        </select>
                        @error('hari')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="kegiatan" id="kegiatan" rows="4" required
                                  ...
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('kegiatan') border-red-500 @enderror">{{ old('kegiatan', $kegiatan->kegiatan) }}</textarea>
                        @error('kegiatan')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        @if($kegiatan->foto_dokumentasi)
                            <div id="currentPhotoArea" class="mb-4 p-4 bg-gray-50 rounded-lg">
                                <p class="text-sm text-gray-600 mb-3 flex items-center">
                                    <span class="material-icons text-sm mr-1">photo</span>
                                    Foto saat ini:
                                </p>
                                <div class="relative inline-block">
                                    <img src="{{ $kegiatan->foto_url }}"
                                         alt="Foto dokumentasi saat ini"
                                ...
                                <p class="text-xs text-gray-500 mt-2">Klik X untuk menghapus foto ini</p>
                            </div>
                        @endif
                            ...
                            <div id="uploadArea"
                                ...
                                <div id="uploadContent">
                                    ...
                                    <p class="text-sm text-gray-600 mb-1">
                                        {{ $kegiatan->foto_dokumentasi ? 'Klik untuk mengganti foto dokumentasi' : 'Klik untuk upload foto dokumentasi' }}
                                    </p>
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            {{ $kegiatan->foto_dokumentasi ? 'Upload foto baru untuk mengganti foto yang ada (opsional)' : 'Upload foto dokumentasi kegiatan (opsional)' }}
                        </p>
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit"
                        ...
                        <span class="material-icons text-sm mr-2 inline">update</span>
                        Update Kegiatan
                    </button>
                </div>
            </form>
    ...
    <script>
        ...
        function removeCurrentPhoto() {
            ...
            if (confirm('Apakah Anda yakin ingin menghapus foto ini?')) {
                currentPhotoArea.style.display = 'none';
